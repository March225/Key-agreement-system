...
1. 数据编解码模块 
   ...
   数据编解码模块使用的编解码技术是[**ASN.1**](https://blog.csdn.net/dongdong7_77/article/details/113072896)
	...
	- ASN.1定义：ASN.1（抽象语法标记）描述了一种对数据进行表示，编码，传输和解码的数据格式，提供了一套完整的描述对象的结构。
	- 进行数据编解码的原因：由于跨平台传输过程中可能存在操作系统不同，大小端模式不同，字节对齐不同，开发语言不同等各种原因，所以需要对数据进行序列化处理。
	- 选择ANS.1进行编解码，而不是JSON或XML的原因：ANS.1是比JSON、XML更高级的语言，相较于它们更为规范。
	- 将ASN.1编解码函数封装成类方便调用
